...
class ConfigManager {
    ...
    var config: KazzUtilsConfig? = null
    private var lastSaveTime = 0L
    val features get() = jsonHolder[ConfigFileType.FEATURES] as KazzUtilsConfig
    lateinit var processor: MoulConfigProcessor<KazzUtilsConfig>
...
//        UpdateManager.injectConfigProcessor(processor)
    ...
    private fun tryReadConfig() {
        try {
            ...
            config = gson.fromJson(builder.toString(), KazzUtilsConfig::class.java)
        } catch (e: Exception) {
...
enum class ConfigFileType(val fileName: String, val clazz: Class<*>) {
    FEATURES("config", KazzUtilsConfig::class.java);
